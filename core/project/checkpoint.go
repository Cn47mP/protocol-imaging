...
import (
	...
	"github.com/Cn47mP/protocol-imaging/core/capture"
...
func (plan PlanDocument) Validate() error {
	...
	for index, adjacency := range plan.RequiredAdjacencies {
		...
		key := adjacency.FromTile + "\x00" + adjacency.ToTile + "\x00" + adjacency.Axis
...
func adjacencyKey(adjacency TileAdjacency) string {
	return adjacency.FromTile + "\x00" + adjacency.ToTile + "\x00" + adjacency.Axis
}
