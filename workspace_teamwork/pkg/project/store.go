...
// CommitCaptureStep satisfies capture engine checkpoint commit requests.
func (session *Session) CommitCaptureStep(ctx context.Context, commit capture.EngineCheckpointCommit) error {
	boundary, err := NewBoundaryFromFrontier(commit.Frontier, 1.0)
	if err != nil {
		return fmt.Errorf("create boundary from frontier: %w", err)
	}
	planID := fmt.Sprintf("plan-%04d", commit.Frontier.Revision)
	supersedes := ""
	if session.ActivePlan() != nil {
		supersedes = PlanArchivePath(session.ActivePlan().ID)
	}
	plan := PlanDocument{
		SchemaVersion: PlanSchemaVersion,
		ID:            planID,
		CreatedAt:     commit.UpdatedAt,
		Supersedes:    supersedes,
		Trigger:       "frontier_step",
		Frontier:      commit.Frontier,
		AcceptableOverlap: OverlapRange{
			Minimum: 0.1,
			Maximum: 0.9,
		},
	}
	plan.RequiredAdjacencies = RequiredAdjacenciesFromFrontier(commit.Frontier)
	plan.TileChecksums = make(map[string]string, len(commit.Frontier.Tiles))
	if prior := session.ActivePlan(); prior != nil {
		for tileID, checksum := range prior.TileChecksums {
			plan.TileChecksums[tileID] = checksum
		}
	}
	if len(commit.TilePNG) > 0 && commit.TileID != "" {
		digest := sha256.Sum256(commit.TilePNG)
		plan.TileChecksums[commit.TileID] = hex.EncodeToString(digest[:])
	}
	var tiles []TileCommit
	if len(commit.TilePNG) > 0 && commit.TilePath != "" {
		tiles = append(tiles, TileCommit{
			ID:   commit.TileID,
			Path: commit.TilePath,
			PNG:  commit.TilePNG,
		})
	}
	return session.CommitCheckpoint(ctx, CheckpointCommit{
		Plan:              plan,
		Boundary:          boundary,
		Tiles:             tiles,
		CaptureState:      CaptureState(commit.CaptureState),
		ActiveCalibration: commit.ActiveCalibration,
		UpdatedAt:         commit.UpdatedAt,
	})
}

// UpdateCaptureStateString allows string capture state updates for generic callers.
func (session *Session) UpdateCaptureStateString(ctx context.Context, state string, activeCalibration string, updatedAt time.Time) error {
	return session.UpdateCaptureState(ctx, CaptureState(state), activeCalibration, updatedAt)
}
